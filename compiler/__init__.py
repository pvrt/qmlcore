...
class EventHandler(FileSystemEventHandler):
  ...
  def _check_event(self, event):
    if not event.is_directory and self._check_file(event.src_path):
      self.modified = True

  @staticmethod
  def _check_file(filepath):
    _, ext = os.path.splitext(filepath)
    return ext in {".qml", ".js"}
  ...
  try:
    while True:
      try:
        ...
        if not wait:
          _handle_compile_exception(ex, verbose)
          sys.exit(1)
      ...
      while True:
        time.sleep(0.1)
        if event_handler and event_handler.pop():
          break
  finally:
    if observer:
      observer.stop()
...
def _handle_compile_exception(ex, verbose):
  if hasattr(ex, 'filename'):
    loc = f"{ex.filename}: "
    if hasattr(ex, 'lineno'):
      loc += f"{ex.lineno}:{ex.col}: "
  else:
    loc = ''
  msg = f"{loc}error: {ex}"
  if hasattr(ex, 'line'):
